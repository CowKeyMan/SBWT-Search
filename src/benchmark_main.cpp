...
auto main(int argc, char **argv) -> int {
  ...
  if (arguments.count("-QUERY_FILE_PARSER_FASTA_ZIP")) query_file_parser_fasta_zip();

  if (arguments.count("-QUERY_FILE_PARSER_FASTQ")) query_file_parser_fastq();
  if (arguments.count("-QUERY_FILE_PARSER_FASTQ_ZIP")) query_file_parser_fastq_zip();

  // RawSequencesParser module tests
  if (arguments.count("-RAW_SEQUENCE_PARSER_CHAR_TO_BITS")) raw_sequence_parser_char_to_bits();
}
