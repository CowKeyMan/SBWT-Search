...
#define BENCHMARK_QUERY_READER
  ...
  cout << "QueryReader_parse_kseqpp_streams: " << TIME_IT_TOTAL << '\n';

  TIME_IT(QueryReader_parse_kseqpp_read());
  cout << "QueryReader_parse_kseqpp_read: " << TIME_IT_TOTAL << '\n';

  TIME_IT(QueryReader_parse_kseqpp_gz_stream());
  cout << "QueryReader_parse_kseqpp_gz_stream: " << TIME_IT_TOTAL << '\n';

  TIME_IT(QueryReader_parse_kseqpp_gz_read());
  cout << "QueryReader_parse_kseqpp_gz_read: " << TIME_IT_TOTAL << '\n';
...
auto QueryReader_parse_kseqpp_gz_read() -> void {
  auto query_reader = QueryReader(fna_filename, 30);
  query_reader.parse_kseqpp_gz_read();
}
